"""EPA Air Quality Index Analysis

Queries the Iceberg epa_aqi_daily table, computes summary statistics,
and writes Parquet output for the Evidence dashboard.
"""
import os, sys
sys.path.insert(0, "/bedrock_sdk")
from job import BedrockJob
...
import adbc_driver_flightclient.dbapi as flight
conn = flight.connect(os.environ.get("BEDROCK_QUERY_ENGINE", "grpc://bedrock-query-engine:7778"),
                      db_kwargs={"username": "unused",
                                 "password": os.environ.get("BEDROCK_TOKEN", ""),
                                 "adbc.flight.sql.rpc.call_header.x-bedrock-token":
                                     os.environ.get("BEDROCK_TOKEN", "")})
cur = conn.cursor()
...
cur.execute(f"""
    ...
    ORDER BY days DESC
""")
pollutants = cur.fetchall()
job.progress(55, "Pollutant analysis done")

# ── 4. Top 30 worst counties (most unhealthy days) ───────────────────────
cur.execute(f"""
...
job.progress(95, "Finalising report…")
